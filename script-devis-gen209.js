...
function updatePricesAndTotal(isRadio4Or5Selected, isRadio1To3Selected) {
    ...
    if (eventEndHour < eventStartHour) eventEndHour += 24; // Adjust for events that end after midnight
    ...
    // Adjust security staff timing based on event timing and requirements
    let securityArrival, securityDeparture;
    if (endHour >= 22 || endHour < 6) { // If event ends between 22h and 6h, security is needed
        securityArrival = eventStartHour < 18 ? 17.5 : eventStartHour - 0.5;
        securityDeparture = eventEndHour + 0.5;
    } else {
        securityArrival = securityDeparture = null; // No security needed
    }

    // Security staff cost calculation with no negative values
    const securityStaffCost = securityArrival !== null && securityDeparture !== null ? numberOfSecurityStaff * 35 * (securityDeparture - securityArrival) : 0;
    ...
    $('#temps-staff-securite').text(securityArrival !== null ? `Le staff sécurité arrivera à ${formatTime(securityArrival)} et partira à ${formatTime(securityDeparture)}. Pour un total de ${securityStaffCost.toFixed(2)}€.` : "Pas de sécurité requise.");
