...
$(document).ready(function() {
    updatePricesAndTotal($('#Traiteur-n-1').is(':checked'), !$('#Traiteur-n-1').is(':checked'));
...
// Adjust staff times and calculate costs
let securityArrival, securityDeparture;
if (isEventAfter22h00(eventTimeString) || startHour < 6) {
    securityArrival = 17.5; // Security starts at 17:30 if event is before 18:00 or if event runs between 22h-6h
} else {
    securityArrival = eventStartHour - 0.5; // 30 minutes before event otherwise
}
securityDeparture = eventEndHour + 0.5;

const securityStaffCost = numberOfSecurityStaff * 35 * (securityDeparture - securityArrival);
const regisseurCost = numberOfRegisseurs * 40 * ((eventEndHour + 1) - (eventStartHour - (isRadio1To3Selected ? 2 : 1)));
const cateringStaffCost = isRadio1To3Selected ? numberOfCateringStaff * YOUR_DEFAULT_CATERING_STAFF_COST * ((eventEndHour + 1) - (eventStartHour - 2)) : 0;
...
$('#temps-staff-traiteur').text(isRadio1To3Selected ? `Le staff traiteur arrivera à ${formatTime(eventStartHour - 2)} et partira à ${formatTime(eventEndHour + 1)}. Pour un total de ${cateringStaffCost.toFixed(2)}€.` : "");
$('#temps-regisseur').text(`Le staff régisseur arrivera à ${formatTime(eventStartHour - (isRadio1To3Selected ? 2 : 1))} et partira à ${formatTime(eventEndHour + 1)}. Pour un total de ${regisseurCost.toFixed(2)}€.`);
